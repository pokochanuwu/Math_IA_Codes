 ...
 * @returns Array
 */
 export async function read(area:string, sheetname:string, filename:string) {
    const book = readFile(filename);
    const ws = book.Sheets[sheetname];
    let arr:Array<any> = []
    let decodeRange = await getdecodeRange(area)
    for (let colIdx:any = decodeRange.s.c, m=0; colIdx <= decodeRange.e.c; colIdx++, m++) {
        arr[colIdx] = []
 ...
 */
export async function write(data:Array<any>, area:string, sheetname:string, filename:string) {
    const book = readFile(filename);
    const ws = book.Sheets[sheetname];
 ...
 */
async function getdecodeRange(range:any) {
    return utils.decode_range(range);
}
async function getencodeRange(a:any) {
